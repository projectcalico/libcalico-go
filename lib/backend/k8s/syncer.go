...
func newResourceState() resourceState {
	return resourceState{Tracker: map[string]model.Key{}}
}
...
func newSyncer(kubeAPI kubeAPI, converter converter, callbacks api.SyncerCallbacks, disableNodePoll bool) *kubeSyncer {
	syn := &kubeSyncer{
		...
		state: map[string]resourceState{
			KEY_NS:  newResourceState(),
			KEY_PO:  newResourceState(),
			KEY_NP:  newResourceState(),
			KEY_SNP: newResourceState(),
			KEY_GC:  newResourceState(),
			KEY_HC:  newResourceState(),
			KEY_IP:  newResourceState(),
			KEY_NO:  newResourceState(),
			KEY_RS:  newResourceState(),
		},
...
// deletes for any KVPairs which we don't know about.  All of the provided KVPs
// should have been generated by the provided resourceType.
func (syn *kubeSyncer) convertKVPairsToUpdates(kvps []model.KVPair, resourceType string) []api.Update {
	updates := []api.Update{}
	for _, kvp := range kvps {
		if _, ok := syn.state[resourceType].Tracker[kvp.Key.String()]; !ok && kvp.Value == nil {
...
func (syn *kubeSyncer) updateTracker(updates []api.Update, resourceType string) {
	for _, upd := range updates {
		...
		if upd.UpdateType == api.UpdateTypeKVDeleted {
			...
			delete(syn.state[resourceType].Tracker, upd.KVPair.Key.String())
		} else {
			log.Debugf("Update tracker: %+v: %+v", upd.KVPair.Key, upd.KVPair.Revision)
			syn.state[resourceType].Tracker[upd.KVPair.Key.String()] = upd.KVPair.Key
		}
	}
}
...
func (syn *kubeSyncer) getUpdateType(kvp model.KVPair, resourceType string) api.UpdateType {
	...
	// Not a delete.
	if _, ok := syn.state[resourceType].Tracker[kvp.Key.String()]; !ok {
...
// Resource types.
// Used to identify various bits of state stored on a per-resource basis.
const (
	KEY_NS  = "Namespace"
	KEY_PO  = "Pod"
...
func (syn *kubeSyncer) performSnapshotDeletes(existsMap map[string]map[string]bool) {
	...
	for resourceType, exists := range existsMap {
		...
		for cachedKey, k := range syn.state[resourceKey].Tracker {
...
// for each.
func (syn *kubeSyncer) performSnapshot() (map[string][]model.KVPair, map[string]map[string]bool) {
	...
	for {
		...
		if syn.needsResync[KEY_NS] {
			...
			syn.state[KEY_NS].LatestVersion = nsList.ListMeta.ResourceVersion
		...
		if syn.needsResync[KEY_NP] {
			...
			syn.state[KEY_NP].LatestVersion = npList.ListMeta.ResourceVersion
		...
		if syn.needsResync[KEY_SNP] {
			...
			syn.State[KEY_SNP].LatestVersion = snpList.Metadata.ResourceVersion
		...
		if syn.needsResync[KEY_PO] {
			...
			syn.state[KEY_PO].LatestVersion = poList.ListMeta.ResourceVersion
		...
		if syn.needsResync[KEY_GC] {
			...
			syn.state[KEY_GC].LatestVersion = resourceVersion
		...
		if syn.needsResync[KEY_IP] {
			...
			syn.state[KEY_IP].LatestVersion = resourceVersion
		...
		if !syn.disableNodePoll && syn.needsResync[KEY_NO] {
			...
			syn.state[KEY_NO].LatestVersion = noList.ListMeta.ResourceVersion
			for _, no := range noList.Items {
				node, err := resources.K8sNodeToCalico(&no)
				if err != nil {
					log.WithError(err).Fatal("Error converting node resource")
				}
				if node != nil {
		...
		log.Infof("Snapshot resourceVersions: %+v", "TODO")
