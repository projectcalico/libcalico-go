...
func KeyFromDefaultPath(path string) Key {
	if m := matchWorkloadEndpoint.FindStringSubmatch(path); m != nil {
		...
		if namespace.IsNamespaced(ri.kind) {
			log.Infof("(BUG) Path is a global resource, but resource is namespaced: %v", path)
			return nil
		}
		...
		ri := resourceInfoByPlural[unescapeName(m[1])]
		if !namespace.IsNamespaced(ri.kind) {
			log.Infof("(BUG) Path is a namespaced resource, but resource is global: %v", path)
			return nil
		}
		log.Infof("Path is a namespaced resource: %v", path)
		return ResourceKey{
