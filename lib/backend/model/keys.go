...
// ListOptionsLastSegmentIsPrefix returns true if the final segment of the default path
// root is a name prefix rather than the full name.
func ListOptionsLastSegmentIsPrefix(listOptions ListInterface) bool {
	// Only supported for ResourceListOptions.
	rl, ok := listOptions.(ResourceListOptions)
	return ok && rl.ListOptionsLastSegmentIsPrefix()
}
