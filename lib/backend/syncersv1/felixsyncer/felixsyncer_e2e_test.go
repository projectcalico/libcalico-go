...
func calculateDefaultCacheEntries(cs kubernetes.Interface, dt apiconfig.DatastoreType, isRemote bool) (expected []model.KVPair) {
	...
	// If this is the Kubernetes datastore then add the default profile (a previous remnant of EE). Again, no labels
	// for this profile.
	if dt == apiconfig.Kubernetes {
		...
		for _, eps := range epss.Items {
			...
			eps.ResourceVersion = ""
			epskv, err := conv.EndpointSliceToKVP(&eps)
			Expect(err).NotTo(HaveOccurred())
...
var _ = testutils.E2eDatastoreDescribe("Felix syncer tests", testutils.DatastoreAll, func(config apiconfig.CalicoAPIConfig) {
	...
	Describe("Felix syncer functionality", func() {
		It("should receive the synced after return all current data", func() {
			...
			defaultCacheEntries := calculateDefaultCacheEntries(cs, config.Spec.DatastoreType, false)
			...
			// or out control (and a lot of the tests below are focussed on host data), instead the tests below will
