...
//
func NewFirstAndIntervalLogger(interval time.Duration) *FirstAndIntervalLogger {
...
type FirstAndIntervalLogger struct {
	// Unix time (ns) of last processed log.
	lastLog  int64
...
func (logger *FirstAndIntervalLogger) logEntry() *logrus.Entry {
	...
	if logger.force || now-logger.lastLog > int64(logger.interval) {
		entry := logger.entry.WithFields(logrus.Fields{
			"log-summary-skipped":     logger.skipped,
			"log-summary-previous-ns": now,
		})
		logger.force = false
		logger.lastLog = now
		logger.skipped = 0
...
// Add an error as single field (using the key defined in ErrorKey) to the FirstAndIntervalLogger.
...
// Add a single field to the FirstAndIntervalLogger.
...
// Add a map of fields to the FirstAndIntervalLogger.
