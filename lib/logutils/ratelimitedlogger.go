...
// RateLimitedLoggerConfig defines the
type RateLimitedLoggerConfig struct {
	// Defaults to 5 minutes if not specified.
	Interval time.Duration

	// Defaults to the standard logger if not specified.
...
// the last log request. Processed logs contain information about the number of skipped logs and the expected earliest
// time of the next log. Force() may be used to force the next log to be processed.  Note that sufficient log level is
// still required for a processed log to actually be written.
//
// Typical use might be as follows:
//
//   Logger := NewRateLimitedLogger(nil).WithField("key": "my-key")
//   for {
...
func NewRateLimitedLogger(config *RateLimitedLoggerConfig) *RateLimitedLogger {
