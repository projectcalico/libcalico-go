...
type FelixConfigurationSpec struct {
	...
	// port should be specified as tcp:<cidr>:<port-number> or udp:<cidr>:<port-number>. For IPv6 networks, the CIDR must also be specified between
	// square brackets (e.g. tcp:[<cidr>]:<port-number>). For back-compatibility, if the protocol is not specified, it defaults to "tcp". If a CIDR
	// is not specified, it will allow traffic from all addresses. To disable all inbound host ports, use the value none. The default value allows
	// ssh access and DHCP.
	...
	// should be specified as tcp:<port-number> or udp:<port-number>. For IPv6 networks, the CIDR must also be specified between square brackets
	// (e.g. tcp:[<cidr>]:<port-number>). For back-compatibility, if the protocol is not specified, it defaults to "tcp". If a CIDR is not specified,
	// it will allow traffic from all addresses. To disable all outbound host ports, use the value none. The default value opens etcd's standard
